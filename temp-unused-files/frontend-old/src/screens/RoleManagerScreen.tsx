...
const RoleManagerScreen = () => {
  ...
  // Delete disabled - roles use isActive flag for deactivation
  const handleDeactivateRole = (role: Role) => {
    if (role.isSystemRole) {
      Alert.alert('Error', 'System roles cannot be deactivated');
      return;
    }
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              // Update role to set isActive = false
              await rbacService.updateRole(role._id, { isActive: false } as any);
              Alert.alert('Success', 'Role deactivated successfully');
              await loadRoles();
            } catch (error: any) {
              console.error('Error deactivating role:', error);
              Alert.alert(
                'Error',
                error.response?.data?.error || 'Failed to deactivate role'
              );
            }
          },
        },
        ...
        <View style={styles.roleHeader}>
          ...
          {!role.isSystemRole && (
            <View style={styles.roleActions}>
              ...
              <TouchableOpacity
                style={styles.actionButton}
                onPress={() => handleDeactivateRole(role)}
              >
                <Icon name="block" size={20} color="#DC143C" />
              </TouchableOpacity>
            </View>
          )}
        </View>
