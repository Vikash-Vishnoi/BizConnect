...
import { MdSearch, MdSend, MdAttachFile, MdImage, MdRefresh, MdCheckCircle, MdCancel, MdUploadFile, MdInsertDriveFile, MdClose } from 'react-icons/md';
import './Inbox.css';
...
const Inbox = () => {
  ...
  const loadMoreMessages = useCallback(async () => {
    ...
    try {
      ...
      const data = await inboxService.getMessages(selectedConversation._id, { page: nextPage, limit: 9 });
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Show scroll to bottom button when user scrolls up more than 100px from bottom
      setShowScrollButton(distanceFromBottom > 100);
      
      // Load more messages when scrolled within 300px from the top
      if (scrolledFromTop < 300 && hasMoreMessages && !loadingMoreMessages) {
  ...
  const handleConversationClick = async (conversation) => {
    ...
    try {
      ...
      const data = await inboxService.getMessages(conversation._id, { page: 1, limit: 1000 });
      ...
      // Mark as read
      await inboxService.markAsRead(conversation._id);
      
      // Update conversation in list to remove unread count
      ...
      const interval = setInterval(async () => {
        if (conversation._id) {
          const data = await inboxService.getMessages(conversation._id, { page: 1, limit: 1000 });
          const currentLastMessage = conversationMessages[conversationMessages.length - 1];
          const newLastMessage = data.messages?.[data.messages.length - 1];
          
          // Only update if there's a new message
          if (newLastMessage && currentLastMessage?._id !== newLastMessage._id) {
            setConversationMessages(data.messages || []);
          }
        }
      }, 5000);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      // Reload messages
      const data = await inboxService.getMessages(selectedConversation._id, { limit: 1000 });
      setConversationMessages(data.messages || []);
                ...
                <div className="conversation-header-bar">
                  <div className="conversation-info">
                    <div className="conversation-avatar-container">
                      <div className="conversation-avatar">
                        {selectedConversation.contact?.profilePhoto ? (
                          <img src={selectedConversation.contact.profilePhoto} alt={selectedConversation.contact.name} />
                        ) : (
                          <div className="avatar-placeholder">
                          ...
                          <div className="message-bubble">
                                ...
                                <div className="message-meta-inline">
                                  ...
                                  {message.direction === 'outgoing' && (
                                    <span className="message-status">
                                      ...
                                      {message.status === 'failed' && <MdCancel size={14} color="#ef4444" />}
                                    </span>
                                  )}
                                </div>
                              </div>
                            ...
                            {message.type === 'image' && (
                              <div className="message-media">
                                <img src={message.content?.mediaUrl} alt="Sent" className="message-image" />
