...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('📊 Setup status from backend:', data.setupStatus);
        console.log('👤 User data:', data.user);
        
        // Manually set auth state since we already have the token and user
        const userWithRole = {
          ...data.user,
          role: data.user.userType || data.user.role || 'normal_user'
        };
        localStorage.setItem('token', data.token);
        localStorage.setItem('user', JSON.stringify(userWithRole));
        
        // Use setup status from backend to determine redirect
        const redirectPath = data.setupStatus?.redirectTo || '/dashboard';
        console.log('📍 Login - Redirecting to:', redirectPath, 'Setup status:', data.setupStatus);
  ...
  return (
    <div className="auth-container" style={{ background: 'linear-gradient(135deg, #6366F1 0%, #4F46E5 100%)' }}>
      ...
      <div className="auth-right" style={{ background: '#F9FAFB' }}>
        <div className="auth-card" style={{ background: 'white', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.1)' }}>
          <div className="auth-header">
            <h2 style={{ color: '#111827' }}>Welcome Back</h2>
            <p style={{ color: '#6B7280' }}>Sign in to continue to your account</p>
          </div>
