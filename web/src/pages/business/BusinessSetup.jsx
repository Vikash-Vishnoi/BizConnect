...
const BusinessSetup = () => {
  ...
  const loadFormData = () => {
    const savedData = localStorage.getItem('businessSetupFormData');
    const businessName = localStorage.getItem('registeredBusinessName');
    ...
    return {
      ...
      businessAccountId: '',
      appSecret: '',
      verifyToken: `verify_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      sendRate: 70
    };
  };
  ...
  useEffect(() => {
    localStorage.setItem('businessSetupFormData', JSON.stringify(formData));
  }, [formData]);
  
  // Load business name from localStorage if it changes
  useEffect(() => {
    const businessName = localStorage.getItem('registeredBusinessName');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.removeItem('businessSetupFormData');
