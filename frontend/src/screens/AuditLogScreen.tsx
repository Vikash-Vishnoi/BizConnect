  ...
  const renderHeader = () => (
    <View style={styles.header}>
      ...
      {stats && (
        ...
        >
          {renderStatCard('Total Actions', stats.totalActions.toLocaleString(), '📊', '#3B82F6')}
          {renderStatCard('Success Rate', `${Math.round((stats.successfulActions / stats.totalActions) * 100)}%`, '✅', '#10B981')}
          {renderStatCard('Unique Users', stats.uniqueUsers, '👥', '#8B5CF6')}
          {renderStatCard('Failed', stats.failedActions, '❌', '#EF4444')}
        </ScrollView>
      )}
