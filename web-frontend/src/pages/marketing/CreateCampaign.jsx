/**
 * @fileoverview Multi-step campaign creation wizard component.
 * 
 * @component CreateCampaign
 * 
 * @description
 * Four-step wizard for creating WhatsApp marketing campaigns with template selection,
 * recipient management, scheduling, and campaign launch. Supports draft saving and
 * auto-saves progress to session storage.
 * 
 * @features
 * - 4-step campaign creation wizard (details, recipients, schedule, review)
 * - Template selection from approved templates
 * - Recipient management (manual entry, CSV upload)
 * - CSV template download with template variables
 * - Campaign scheduling (immediate or scheduled)
 * - Timezone-aware scheduling
 * - Send rate control (1-80 messages/minute)
 * - Draft auto-saving to session storage
 * - Manual draft saving to database
 * - Form validation at each step
 * - Reset confirmation modal
 * 
 * @state
 * - currentStep: Current wizard step (1-4)
 * - campaignData: { name, description, templateId, recipients, schedule, settings }
 * - templates: Available approved templates
 * - selectedTemplate: Currently selected template
 * - uploadedFile: Uploaded CSV file reference
 * 
 * @api
 ...
 * - Redirects to /campaigns/:id after creation
 * 
 * @example
 * // Route configuration
...
/**
 * Session storage key for campaign draft auto-save
 * @constant {string}
 */
const STORAGE_KEY = 'campaign-draft';

/**
 * Send rate constraints for campaign sending
 * @constant {Object}
 */
...
const CreateCampaign = () => {
  ...
  /**
   * Loads campaign draft from session storage
   * @returns {Object|null} Saved campaign data or null
   */
  const loadSavedData = () => {
    try {
  ...
  const [campaignData, setCampaignData] = useState(savedData?.campaignData || {
    ...
    settings: {
      sendRate: SEND_RATE_LIMITS.DEFAULT
    }
  });
  ...
  /**
   * Handles template selection and updates campaign data
  ...
  /**
   * Parses manual text input and adds recipients to campaign
   */
  ...
  /**
   * Handles CSV file upload and parses recipients
  ...
  /**
   * Removes a recipient from the campaign
   * @param {number} index - Index of recipient to remove
   */
  ...
  /**
   * Generates and downloads a CSV template with sample data
   */
  ...
  /**
   * Creates and launches the campaign
   */
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      sessionStorage.setItem(STORAGE_KEY, JSON.stringify(campaignData));
      setLoading(false);
    } catch (err) {
          ...
          {currentStep === 3 && (
            <div className="step-container">
              ...
              <div className="create-campaign__form-group">
                ...
                <p className="form-help">Control how many messages are sent per minute ({SEND_RATE_LIMITS.MIN}-{SEND_RATE_LIMITS.MAX})</p>
                <input
                  type="number"
                  className="send-rate-input"
                  min={SEND_RATE_LIMITS.MIN}
                  max={SEND_RATE_LIMITS.MAX}
                  value={campaignData.settings.sendRate}
                  onChange={(e) => {
                    const value = parseInt(e.target.value);
                    if (value < SEND_RATE_LIMITS.MIN) {
                      toast.error(`Send rate must be at least ${SEND_RATE_LIMITS.MIN} message per minute`);
                      return;
                    }
                    if (value > SEND_RATE_LIMITS.MAX) {
                      toast.error(`Send rate cannot exceed ${SEND_RATE_LIMITS.MAX} messages per minute`);
                      return;
                    }
                    setCampaignData(prev => ({
                      ...prev,
                  ...
                  onBlur={(e) => {
                    const value = parseInt(e.target.value);
                    if (isNaN(value) || value < SEND_RATE_LIMITS.MIN) {
                      toast.warning(`Invalid send rate. Setting to default (${SEND_RATE_LIMITS.DEFAULT})`);
                      setCampaignData(prev => ({
                        ...prev,
                        settings: { ...prev.settings, sendRate: SEND_RATE_LIMITS.DEFAULT }
                      }));
                    }
                  }}
                />
