/**
 * ⚡ Automation Flow List Component
 * 
 * Manage chatbot automation flows with triggers and execution tracking.
 * Create, edit, delete, and toggle flow activation status.
 * 
 * @component
 * @features
 * - View all automation flows in grid layout
 * - Create new flows with name, description, trigger type
 * - Edit existing flow configuration
 * - Delete flows with confirmation
 * - Toggle flow active/inactive status
 * - Execution count and success rate tracking
 * - Auto-save form data during editing
 * - Empty state with call-to-action
 * - Status indicators (active/inactive)
 * - Trigger type icons (keyword/webhook/schedule/manual)
 * 
 * @state
 * - flows: Array of automation flow objects
 * - loading: Data fetch loading state
 * - showModal: Create/edit modal visibility
 * - showDeleteModal: Delete confirmation modal
 * - editingFlow: Flow being edited (null for new)
 * - formData: Flow form fields (name, description, trigger, isActive)
 * 
 * @api
 * - GET /flows: Fetch all flows
 ...
 * 
 * @example
 * <Route path="/automation/flows" element={<FlowList />} />
 */
...
/**
 * Auto-save storage key for flow form
 */
const AUTOSAVE_KEY = 'flowForm';

/**
 * Available trigger types for automation flows
 */
const TRIGGER_TYPES = ['keyword', 'webhook', 'schedule', 'manual'];
...
const FlowList = () => {
  ...
  const getTriggerIcon = (trigger) => {
    return TRIGGER_ICONS[trigger] || TRIGGER_ICONS.default;
  };
  ...
  return (
    <>
      ...
      <div className="flow-list-container">
        ...
        {showModal && (
          <div className="flow-list__modal-overlay" onClick={handleCloseModal} role="dialog" aria-modal="true" aria-labelledby="flow-modal-title">
            <div className="flow-list__modal-content" onClick={(e) => e.stopPropagation()} role="document">
              <div className="flow-list__modal-header">
                <h2 id="flow-modal-title">{editingFlow ? 'Edit Flow' : 'Create New Flow'}</h2>
                <button className="flow-list__close-btn" onClick={handleCloseModal} aria-label="Close modal">×</button>
              </div>
